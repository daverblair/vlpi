...
from vlpi.AnomalyDetection import PheRS_AnomalyDetection,JaccardSimilarity,EmbeddingAnomalyDetection
...
class vLPI:
    ...
    def _parseArgsContinuous(self,**kwargs):
        ...
        if 'ContinuousModelParameters' not in allKeywordArgs:
            ...
            modelParameters['anchorDxPriors']={'anchorDxNoise':[1.0,1.0],'latentDimToAnchorDxMap':1.0,'prevalence':[0.000001,0.1]}
    ...
    def _parseArgsDiscrete(self,**kwargs):
        ...
        if 'DiscreteModelParameters' not in allKeywordArgs:
            ...
            modelParameters['latentPhenotypePriors']={'element_wise_precision':[1.0,1.0],'prevalence':[0.000001,0.1]}
            modelParameters['covariatePriors']={'intercept':[0.0,5.0],'cov_scale':3.0}
        else:
    ...
    def SimulateData(self, numSamples,numAssociatedTraits,numMaskedTraits = 0, numNoiseTraits = 0,numCategoricalCovariates=[],**kwargs):
        ...
        if simModelType=='Continuous':
            ...
            if self.useAvailableAnchorDx is None:
                tmpPrev=infer_liability_CI(modelParameters['anchorDxPriors']['prevalence'])
                lp_prev = dist.Normal(torch.tensor(tmpPrev[0]),torch.tensor(tmpPrev[1])).sample()
...
#        return log_loss_train,log_loss_test
    ...
    def FitModel(self,datasetSampler,batch_size=0,verbose=True,**kwargs):
        ...
        if 'initializationType' in allKeywordArgs:
            initializationType=kwargs['initializationType']
            assert initializationType in ['PheRS','Embedding'],"Only PheRS and Embedding-based initializations supported."
        else:
            if self.modelType=='Discrete':
                initializationType='PheRS'
            else:
                initializationType='Embedding'
        ...
        cosineAnnealingDict = {'withCosineAnnealing':withCosineAnealing,'initialRestartInterval':20,'intervalMultiplier':2}
        ...
        if self.modelType!='Discriminative' and alreadyInitialized==False:
            ...
            if initializationType=='PheRS':
                assert ((self.modelType=='Continuous') and (modelParameters['nLatentDim']>1)) is False,"Cannot use PheRS to initialize a multidimensional Continuous Phenotype model."
                trainScores,testScores = self.ComputePheRS(datasetSampler)
            else:
                try:
            ...
            #optimize guide only
            if batch_size > 0:
                output=optimizer.BatchTrain(batch_size,errorTol=initializationErrorTol,optimizationStrategy='GuideOnly',verbose=verbose)
            else:
                output=optimizer.FullDatasetTrain(errorTol=initializationErrorTol,optimizationStrategy='GuideOnly',verbose=verbose)
...
#                
    ...
    def ComputePheRS(self,datasetSampler):
        previousArrayType = datasetSampler.returnArrays
        if datasetSampler.returnArrays!='Sparse':
            datasetSampler.ChangeArrayType('Sparse')
        ...
        phe_rs = PheRS_AnomalyDetection(sparseTrainingData,sparseTestingData)
        trainScores,testScores = phe_rs.ComputeScores()
    ...
    def ComputeJaccardSimilarity(self,datasetSampler,index_vec):
        ...
        sparseTestingData=datasetSampler.ReturnFullTestingDataset(randomize=False)[0]
        jaccard = JaccardSimilarity(sparseTrainingData,sparseTestingData,index_vec_binary)
        trainScores,testScores = jaccard.ComputeScores()
        
        if previousArrayType!='Sparse':
            datasetSampler.ChangeArrayType(previousArrayType)
        return trainScores,testScores
    
    def ComputeAnomalyScores(self,datasetSampler,num_components=1,embedding_type='nmf'):
        previousArrayType = datasetSampler.returnArrays
        if datasetSampler.returnArrays!='Sparse':
            datasetSampler.ChangeArrayType('Sparse')
        sparseTrainingData=datasetSampler.ReturnFullTrainingDataset(randomize=False)[0]
        ...
        sparseTestingData=datasetSampler.ReturnFullTestingDataset(randomize=False)[0]
        embed=EmbeddingAnomalyDetection(sparseTrainingData,sparseTestingData,num_components=num_components,embedding_model=embedding_type)
        if previousArrayType!='Sparse':
            datasetSampler.ChangeArrayType(previousArrayType)
        return embed.ComputeReconstructionScores(num_components)
    
    def ComputeEmbeddings(self,datasetSampler,num_components=1,embedding_type='NMF'):
        previousArrayType = datasetSampler.returnArrays
        if datasetSampler.returnArrays!='Sparse':
            datasetSampler.ChangeArrayType('Sparse')
        sparseTrainingData=datasetSampler.ReturnFullTrainingDataset(randomize=False)[0]
        ...
        if previousArrayType!='Sparse':
            datasetSampler.ChangeArrayType(previousArrayType)
        return embed.ComputeEmbeddings(num_components)
...
if __name__=='__main__':
    ...
    continuousModelParams['anchorDxPriors']={'anchorDxNoise':[1.0,10.0],'latentDimToAnchorDxMap':0.1,'prevalence':[0.05,0.1]}
    continuousModelParams['latentPhenotypePriors']={'element_wise_precision':[1,2.0]}
    continuousModelParams['covariatePriors']={'intercept':[-3.0,1.0],'cov_scale':0.025}
    
    discreteModelParams={}
    discreteModelParams['anchorDxPriors']={'anchorDxNoise':[1.0,10.0]}
    discreteModelParams['latentPhenotypePriors']={'element_wise_precision':[1,2.0],'prevalence':[0.05,0.1]}
    discreteModelParams['covariatePriors']={'intercept':[-3.0,1.0],'cov_scale':0.025}
    ...
    test_vlpi=vLPI(modelType,useAvailableAnchorDx=useAvailableAnchorDx,neuralNetworkHyperparameters={'n_layers' : 2, 'n_hidden' : 32, 'dropout_rate': 0.2, 'use_batch_norm':True})
    ...
    trainPheRSScores,testPheRSScores = test_vlpi.ComputePheRS(sampler)
    trainPCAScores,testPCAScores = test_vlpi.ComputeEmbeddings(sampler,num_components=nLatentFitDim)
    
    if modelType=='Continuous':
        continuousModelParams['nLatentDim']=nLatentFitDim
        output=test_vlpi.FitModel(sampler,batch_size=1000,mappingFunction='Linear_Monotonic',learningRate=0.05,withCosineAnnealing=True,initializationErrorTol=1e-2,errorTol=1e-4,maxEpochs=200,ContinuousModelParameters = continuousModelParams,numParticles=10,maxInitializationEpochs=20,alreadyInitialized=False)
    else:
        output=test_vlpi.FitModel(sampler,batch_size=1000,mappingFunction='Linear_Monotonic',learningRate=0.05,withCosineAnnealing=True,initializationErrorTol=1e-2,errorTol=1e-4,maxEpochs=200,numParticles=10,maxInitializationEpochs=20,alreadyInitialized=False)
#        

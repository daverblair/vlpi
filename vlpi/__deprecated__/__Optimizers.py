...
class Optimizers:
    ...
    def _contstructSVI(self,optimizationStrategy):
        ...
        if optimizationStrategy=='ScoreBased':
            ...
            if optimizationStrategy=='PosteriorOnly':
            ...
            elif optimizationStrategy=='ComponentsOnly':
                _model=self.phenotypeModel.model
                _guide=poutine.block(self.phenotypeModel.guide,hide=[x for x in param_store.get_all_param_names() if ('latentPhenotypeEffects' not in x)])
...
#                scheduler = CosineAnnealingLR({'optimizer': AdamW, 'optim_args': AdamWOptimArgs,'T_max': self.maxEpochs})
            else:
                scheduler = ReduceLROnPlateau({'optimizer': Adam, 'optim_args': AdamOptimArgs, 'threshold': self.LRPlateauTol})
                
#                scheduler = CosineAnnealingLR({'optimizer': Adam, 'optim_args': AdamOptimArgs,'T_max': self.maxEpochs})
            return {'svi':SVI(_model,_guide,scheduler, loss=Trace_ELBO(num_particles=self.numParticles)),'scheduler':scheduler}
    ...
    def FullDatasetTrain(self, errorTol:float = 1e-5,verbose:bool=True,logFile=None,optimizationStrategy='Full',errorComputationWindow=None):
        ...
        assert optimizationStrategy in ['Full','PosteriorOnly','ScoreBased','EncoderOnly','ComponentsOnly'],"Available Optimization strategies: 'Full','PosteriorOnly','ScoreBased','EncoderOnly','ComponentsOnly'"
